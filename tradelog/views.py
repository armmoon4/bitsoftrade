...
def _is_trade_fully_tagged(trade):
    ...
    A trade is considered fully tagged when ALL of the following are present:
      ...
      - at least one rule tagged  (via trade_rules related manager OR rules_followed JSONField)
      - at least one mistake/violation tagged (via trade_mistakes related manager OR violation_modes JSONField)

    Called from:
      - TradeListCreateView.perform_create
      - TradeDetailView.perform_update
      - TradeTagUpdateView.patch  (rules/mistakes tagging API)
    """
    ...
    # Rules: tagged via related model OR directly via JSONField
    has_rules = (
        trade.trade_rules.exists()
        or bool(trade.rules_followed)
    )

    # Mistakes: tagged via related model OR directly via violation_modes JSONField
    has_mistakes = (
        trade.trade_mistakes.exists()
        or bool(trade.violation_modes)
    )
